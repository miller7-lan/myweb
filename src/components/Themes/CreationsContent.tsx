...
const projects: Project[] = [
  ...
  {
    title: '利润助手',
    subtitle: 'Local Profit Ledger',
    icon: Calculator,
    desc: '本地极简家庭利润记账软件，围绕录入、看板、历史和设置构建，数据保存在本机 SQLite 文件中。',
    requirements: [
      '本地运行，账目数据不上云',
      '支持自定义字段和可配置利润计算公式',
      '用安全公式引擎限制表达式能力，避免 eval 注入风险',
    ],
    problem: '把家庭利润记录从零散表格变成一个本地、可配置、能看趋势的轻量系统。',
    design: [
      '用 SQLite 单文件保证本地私有和迁移方便',
      '通过 EventBus 让录入、历史和看板在数据变更后自动刷新',
      '用 AST 白名单实现安全公式，支持中文字段又避免代码执行风险',
    ],
